...
class Normalize(object):
    ...
    def __call__(self, sample):
        img1 = np.array(sample['image1']).astype(np.float32)
        img2 = np.array(sample['image2']).astype(np.float32)
        mask = np.array(sample['label']).astype(np.float32)
        flow = np.array(sample['flow']).astype(np.float32)

        img1 = ((img1/255)-self.mean)/self.std
        img2 = ((img2/255)-self.mean)/self.std
        flow = ((flow/255)-self.mean)/self.std
        # import pdb; pdb.set_trace()

        return {'image1': img1, 'image2': img2, 'label': mask, 'flow': flow}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        img1 = np.array(sample['image1']).astype(np.float32).transpose((2, 0, 1))
        img2 = np.array(sample['image2']).astype(np.float32).transpose((2, 0, 1))
